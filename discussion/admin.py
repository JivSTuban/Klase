...
from .models import InstructorDiscussion, StudentDiscussion
...
def make_draft(InstructorDiscussion, request, queryset):
    queryset.update(status='0')

@admin.action(description='Change to published')
def make_published(InstructorDiscussion, request, queryset):
    queryset.update(status='1')

@admin.action(description='Change to archived')
def make_archived(InstructorDiscussion, request, queryset):
    queryset.update(status='2')
...
admin.site.register(InstructorDiscussion,Facultyadmin)
